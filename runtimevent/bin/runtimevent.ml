...
let ev_counter tis counter_name = {tis = Int64.to_int(Event.Timestamp.to_int64 tis); counter_name= Event.runtime_counter_name counter_name}
...
let get_phases ?phase_name ()=
  Hashtbl.iter (fun counter_name tis ->
    ...
    if counter_name = "minor_allocated" then Printf.printf "number of counters for '%s' is %d \n" counter_name len
  )counter_dict;
...
let () = 
  if Array.length Sys.argv > 1 then get_phases ~phase_name:(Sys.argv.(1)) () 
  else get_phases ()
